...
We used a model-based double sampling approach to estimate salmon escapement [@deacy_etal_2016], where we used time lapse and video equipment to predict salmon passage for the entire season.  
...
# Results {-}
Based on $\Delta$AICc, the most parsimonious model was the `r tolower(models$aic_table$Modnames[[1]])` model (Table \@ref(tab:topmodel-table), Figure \@ref(fig:topmodel-plot)). Model diagnostics indicated normal residuals and constant error variance (homoskedasticity). 
...
Here's where discussion text will go.
