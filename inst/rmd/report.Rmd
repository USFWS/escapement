...
# Results {-}
Based on `r $\Delta$AIC`, the most parsimonious model was the `r tolower(models$aic_table$Modnames[[1]])` model (Table \@ref(tab:topmodel-table), Figure \@ref(fig:topmodel-plot)). Model diagnostics indicated normal residuals and constant error variance (homoskedasticity). 
...
knitr::kable(models$aic_table[c(1:4, 6, 8)],
             ...
             col.names = c("Models", "K", expression(Delta*"AICc"), "Delta AICc", "AICc weight", "Cumulative weight"),
             row.names = FALSE
             )
```
