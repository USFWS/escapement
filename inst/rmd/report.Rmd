...
# Abstract
A time-lapse photography and video recording system were used to enumerate adult sockeye salmon (*Oncorhynchus nerka*) entering Akalura Lake. In `r as.character(max(dat$year))`, the system was installed on `r as.character(format(min(subset(dat$date, dat$year == max(dat$year))), "%d %B"))` and operated through `r as.character(format(max(subset(dat$date, dat$year == max(dat$year))), "%d %B, %Y"))`. The time-lapse photography camera recorded 3 sequential pictures at the start of each minute for the duration of the project. Following a double-sampling protocol, a `r tolower(models$aic_table$Modnames[[1]])` model of the relationship between time-lapse and video counts was found to be the most parsimonious mode to predict salmon passage across the season. In `r as.character(peak$year)`, estimated daily salmon passage peaked at `r round(peak$escapement, 0)` on `r as.character(format(peak$day, "%B %d, %Y"))` and by the end of the season a total of `r round(last(boots$summary$escapement), 0)`(95% CI=`r round(last(boots$summary$lower_ci), 0)`--`r round(last(boots$summary$upper_ci), 0)`) sockeye were estimated to have migrated into Akalura Lake.
...
```{r topmodel-table, echo=F, warning=F, fig.align="left", message=F}
models$top_model %>%
  tidy() %>%
...
Using the top model, we estimated daily salmon passage peaked at `r as.character(round(peak$escapement, 0))` salmon on `r as.character(format(peak$day, "%B %d, %Y"))`. Annual escapement was `r round(last(boots$summary$escapement), 0)`(95% CI=`r round(last(boots$summary$lower_ci), 0)`-`r round(last(boots$summary$upper_ci), 0)`) salmon. Escapement in `r as.character(boots$summary[nrow(boots$summary), "year"])` was `r round(boots$summary[nrow(boots$summary), "escapement"], )` salmon compared to `r round(boots$summary[nrow(boots$summary)-1, "escapement"], 0)` salmon in `r as.character(boots$summary[nrow(boots$summary)-1, "year"])` (Table \@ref(tab:summary-table)).
...
```{r aicc-table, echo=FALSE, fig.align="left", message=F}
knitr::kable(models$aic_table,
             digits = 0,
             format = "latex",
             booktabs = TRUE,
             ...
             col.names = c("Models", "K", "AICc", "Delta AICc", "AICc weight", "Cumulative weight", "Log likelihood"),
             format.args = list(big.mark = ","))
```


```{r summary-table, echo=FALSE, fig.align="left", message=F}
